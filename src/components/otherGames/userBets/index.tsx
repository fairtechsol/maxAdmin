...
const OtherUserBets = () => {
  ...
  return (
    <><div>
      <Card className="m-b-10 scorecard responsive-card" style={{ marginBottom: '0px' }}>
        <Card className="m-b-10 my-bet">
          <Card.Header>
            <Nav variant="tabs" defaultActiveKey="#matched-bet">
              <Nav.Item>
                <Nav.Link eventKey="#matched-bet" className="nav-link-a active">
                  Matched {`(${placedBets.length})`}
                </Nav.Link>
              </Nav.Item>
            </Nav>
            <a href="javascript:void(0)" className="nav-link-a btn btn-back float-right" onClick={() => setShowModal(true)}>
              View More
            </a>
          </Card.Header>
          <Card.Body>
            <Tab.Content>
              <Tab.Pane eventKey="#matched-bet" className="active show">
                <div className="table-responsive">
                  <Table id="matched" className="table coupon-table table-bordered table-striped">
                    <thead>
                      <tr>
                        <th>UserName</th>
                    ...
                    <tbody>
                      {placedBets?.length > 0 ? (placedBets?.map((bet: any) => {
                        return (
                          <tr key={bet?.id}>
                            <td
