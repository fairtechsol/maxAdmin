...
const UserBetModalForm = (props: any) => {
  ...
  // }, [searchListData]);
  useEffect(() => {
    const filtered = props.morePlacedBets.filter((item:any) => item?.deleteReason === null);
  ...
  const handleRadioChange=(type:any)=>{
    setInputValue(type);
    if(type==="back"){
      const filtered = props.morePlacedBets.filter((item:any) => item?.betType === "BACK"||item?.betType === "back");
      setFilteredItems(filtered);
    }else if(type==="lay"){
      const filtered = props.morePlacedBets.filter((item:any) => item?.betType === "LAY"||item?.betType === "lay");
      setFilteredItems(filtered);
    }else{
  ...
  return (
    ...
    >
      <div className="d-flex flex-row w-100 align-items-start p-2" style={{backgroundColor:"#f6f6f6"}}>
       <button style={{fontSize:"14px",padding:"6px",backgroundColor:selectedUser?"#556ee6":"#eee",border:0,color:selectedUser?"#fff":"#333",borderRadius:"5px"}} onClick={()=>handleDelete("match")}>Matched Bets</button>
       <button style={{fontSize:"14px",padding:"6px",backgroundColor:selectedUser?"#eee":"#556ee6",border:0,color:selectedUser?"#333":"#fff",borderRadius:"5px"}} onClick={()=>handleDelete("delete")}>Deleted Bets</button>
      </div>
      <div className="d-flex flex-column p-1" style={{border:"1px solid #b6b4b4"}}>
      <div className="row">
